...
@section('content')
    <div class="page_banner">
        <div class="container d-flex justify-content-center flex-column gap-3 h-100">
            <p class="m-0">
                {{ $categoriespreferences['categories_title'] ?? '' }}
            </p>
            <h2 class="mb-0">
                {{ $categoriespreferences['categories_heading'] ?? '' }}
            </h2>
        </div>
    </div>
        ...
        <div class="container">
            <div class="row justify-content-between align-items-center">
                <div class="qhi col-md-4">
                    ...
                    <h6>{{ $categoriespreferences['information_heading1'] ?? '' }}</h6>
                    <p>
                       {{ $categoriespreferences['information_description1'] ?? '' }}
                    </p>
                </div>
                <div class="qhi col-md-4">
                    <img src="{{ asset('media/Help&Support.svg') }}" alt="Help and Support" loading="lazy">
                    <h6>{{ $categoriespreferences['information_heading2'] ?? '' }}</h6>
                    <p>
                        {{ $categoriespreferences['information_description2'] ?? '' }}
                    </p>
                </div>
                <div class="qhi col-md-4">
                    <img src="{{ asset('media/Quick&Install.svg') }}" alt="Quick & easy to install" loading="lazy">
                    <h6>{{ $categoriespreferences['information_heading3'] ?? '' }}</h6>
                    <p>
                       {{ $categoriespreferences['information_Description3'] ?? '' }}
                    </p>
                </div>
            </div>
        </div>

                ...
                <div class="card mb-4">
                    <div class="card-body">
                        <div class="d-sm-flex align-items-center justify-content-between">
                            <h3 class="mb-0 bc-title"><b>Update Website Template</b></h3>
                        ...
                        <div class="card o-hidden border-0 shadow-lg">
                            <div class="card-body">
                                <div class="row justify-content-center">
                                    <div class="col-lg-12">
                                        <form action="{{ route('admin.website-template.update', $template->id) }}"
                                            method="POST">
                                            @csrf
                                                ...
                                                <select name="website_id" class="form-control" id="website_id">
                                                    ...
                                                    @foreach ($websites as $website)
                                                        <option value="{{ $website->id }}"
                                                            {{ old('website_id',$template->website_id) == $website->id ? 'selected' : '' }}>
                                                            {{ $website->domain_name }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                            ...
                                            {{-- <div class="form-group">
                                                ...
                                                <select name="status" id="status" class="form-control">
                                                    <option value="1" {{ $template->status == 1 ? 'selected' : '' }}>
                                                        Active</option>
                                                    <option value="0" {{ $template->status == 0 ? 'selected' : '' }}>
                                                        Inactive</option>
                                                </select>
                                                @error('status')
                                                    <span class="text-danger">{{ $message }}</span>
